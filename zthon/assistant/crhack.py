...
keyboard = [
  ...
  [
    Button.url("سۅٛࢪس ڪࢪستيڼ𓅛", "https://t.me/pp_g3")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        file.write(i + "\n\n**- بواسطـة كرستين @pp_g3**")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس كرستين ❤️** \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\n**- شكـراً لـ استخدامـك سـورس كرستين ❤️**", buttons=keyboard)
